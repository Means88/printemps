...
export function History({projects,en,onOpen,onRefresh,onBack}:{projects:Project[];en:boolean;onOpen:(p:Project)=>void;onRefresh:()=>Promise<void>;onBack:()=>void}){
 ...
 const selected=!showArchived?(filtered.find(item=>item.project.id===selectedId)||filtered[0])?.project:undefined
 return <><main className="history-page">
  ...
  <div className="history-tools"><input aria-label={t('搜索项目','Search projects')} placeholder={t('搜索项目、音频或声部','Search projects, audio or stems')} value={query} onChange={e=>setQuery(e.target.value)}/><select aria-label={t('项目排序','Sort projects')} value={sort} onChange={e=>setSort(e.target.value as 'recent'|'name')}><option value="recent">{t('最近处理','Recently updated')}</option><option value="name">{t('名称','Name')}</option></select><button disabled={busy} aria-pressed={showArchived} onClick={()=>{setShowArchived(!showArchived);setQuery('')}}>{showArchived?t('全部项目','All projects'):t('已删除','Deleted')}</button></div>
  {!deleting&&error&&<p role="alert">{error}</p>}
  <div className={`history-content ${selected?'has-inspector':''}`}><div className="history-list">{filtered.map(item=><div key={item.archiveId||item.project.id} className={`history-row ${selected?.id===item.project.id?'history-row-selected':''}`}>
