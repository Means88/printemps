...
export function History({projects,en,onOpen,onRefresh,onBack}:{projects:Project[];en:boolean;onOpen:(p:Project)=>void;onRefresh:()=>Promise<void>;onBack:()=>void}){
 ...
 return <><main className="history-page">
  ...
  <div className="history-tools"><input aria-label={t('搜索项目','Search projects')} placeholder={t('搜索项目、音频或声部','Search projects, audio or stems')} value={query} onChange={e=>{setQuery(e.target.value);setPage(0)}}/><select aria-label={t('结果筛选','Filter results')} value={resultFilter} onChange={e=>{setResultFilter(e.target.value as 'all'|'results'|'empty');setPage(0)}}><option value="all">{t('全部结果','All results')}</option><option value="results">{t('已有分离结果','Has results')}</option><option value="empty">{t('尚无分离结果','No results')}</option></select><select aria-label={t('项目排序','Sort projects')} value={sort} onChange={e=>{setSort(e.target.value as 'recent'|'name');setPage(0)}}><option value="recent">{t('最近处理','Recently updated')}</option><option value="name">{t('名称','Name')}</option></select><button disabled={busy} aria-pressed={showArchived} onClick={()=>{setShowArchived(!showArchived);setQuery('');setPage(0)}}>{showArchived?t('全部项目','All projects'):t('已删除','Deleted')}</button></div>
  {!deleting&&error&&<p role="alert">{error}</p>}
  <div className={`history-content ${selected?'has-inspector':''}`}><div className="history-list">{result.items.map(item=><div key={item.archiveId||item.project.id} className={`history-row ${selected?.id===item.project.id?'history-row-selected':''}`}>
   {item.archiveId?<div className="history-info"><strong>{item.project.name}</strong><small>{item.project.sourceName} · {item.project.tracks.length} {t('音轨','tracks')}</small><small>{new Date(item.project.updatedAt).toLocaleString(en?'en-US':'zh-CN')}</small></div>:<button aria-pressed={selected?.id===item.project.id} onClick={()=>setSelectedId(item.project.id)} onDoubleClick={()=>onOpen(item.project)}><strong>{item.project.name}</strong><small>{item.project.sourceName} · {item.project.tracks.length} {t('音轨','tracks')}</small><small>{new Date(item.project.updatedAt).toLocaleString(en?'en-US':'zh-CN')}</small></button>}
  ...
  </div>)}{!filtered.length&&<p>{query||resultFilter!=='all'?t('没有匹配项目，请调整关键词或结果筛选。','No matching projects. Adjust your search or result filter.'):showArchived?t('没有保留的已删除项目。','No retained deleted projects.'):t('还没有项目。导入音频即可开始。','No projects yet. Import audio to get started.')}</p>}</div>
 {selected&&<aside className="history-inspector" aria-label={t('记录详情','Project details')}><small>{t('记录详情','Project details')}</small><h2 title={selected.name}>{selected.name}</h2><p title={selected.sourceName}>{selected.sourceName}</p><h3>{t('分离结果','Separation results')} · {selected.tracks.filter(track=>track.role!=='original').length}</h3><ul>{selected.tracks.filter(track=>track.role!=='original').map(track=><li key={track.id}><span title={track.name} style={{borderLeftColor:track.color}}>{track.name}</span><small>{t('可试听','Ready')}</small></li>)}</ul>{selected.tracks.length===1&&<p>{t('尚未分离，可进入工作区选择声部。','No stems yet. Open the workspace to separate parts.')}</p>}<div className="history-inspector-actions"><button className="primary" onClick={()=>onOpen(selected)}>{t('继续试听','Open workspace')}</button><ExportDialog key={selected.id} project={selected} en={en} beforeExport={async()=>{}}/></div></aside>}</div>
 <nav className="history-pagination" aria-label={t('项目分页','Project pages')}><span aria-live="polite">{result.from}–{result.to} / {result.total}</span><button disabled={busy||result.page===0} onClick={()=>setPage(result.page-1)}>{t('上一页','Previous')}</button><button disabled={busy||result.page===result.pages-1} onClick={()=>setPage(result.page+1)}>{t('下一页','Next')}</button></nav>
