import {visibleTrackCount} from '../shared/timeline'
...
import {TrashIcon,BarChartIcon,MagnifyingGlassIcon} from '@radix-ui/react-icons'
...
export function History({projects,en,onOpen,onRefresh,onBack}:{projects:Project[];en:boolean;onOpen:(p:Project)=>void;onRefresh:()=>Promise<void>;onBack:()=>void}){
 ...
 return <><main className="history-page">
  ...
  <div className="history-heading"><div><h1>{showArchived?t('已删除项目','Deleted projects'):t('全部项目','All projects')}</h1><p>{showArchived?t('已删除的项目可以恢复或永久清除。','Deleted projects can be restored or purged permanently.'):t('找到以前处理过的音频，继续试听或导出结果。','Find audio you processed before, then keep auditioning or export the results.')}</p></div><button disabled={busy} aria-pressed={showArchived} onClick={()=>{setShowArchived(!showArchived);setQuery('');setPage(0)}}>{showArchived?t('全部项目','All projects'):t('已删除','Deleted')}</button><button className="primary" onClick={onBack}>{t('新建项目','New project')}</button></div>
  ...
  <small className="history-count">{t('共 ','')}{filtered.length} {t('条记录',filtered.length===1?'record':'records')}</small>
