...
import { clipDuration } from '../shared/clips'
import { formatTimecode } from '../shared/timecode'
function deviceLabel(device:string|undefined,en:boolean){const t=(zh:string,english:string)=>en?english:zh;return device==='cpu'?'CPU':device==='cuda'?'CUDA':device==='mps'?t('MPS（实验性）','MPS (experimental)'):t('自动选择','Automatic')}
...
export function SeparationPanel({project,sourceId,en,onComplete,beforeExport,collapsed,onBusyChange,clipId,onEditClip,editingBlocked}:{project:Project;sourceId:string;en:boolean;onComplete:(id:string)=>void;beforeExport:()=>Promise<void>;collapsed:boolean;clipId:string;onEditClip:(action:ClipAction)=>Promise<void>;onBusyChange:(busy:boolean)=>void;editingBlocked:boolean}){
 ...
 useEffect(()=>{if(!open)return;let live=true;window.printemps.getSettings().then(settings=>{if(live)setDevice(settings.device)}).catch(()=>{});return ()=>{live=false}},[open])
