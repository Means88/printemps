...
export function ModelDownloadProgress({task,project,en}:{task:SeparationTask;project:Project;en:boolean}){
 ...
 const percent=Math.round(Math.max(0,Math.min(1,current?current.received/current.total:task.progress))*100)
  const cachedTargets=(task.targets??[]).filter(id=>!rows.some(row=>row.id===id))
 ...
 return <div className="model-download-layout"><section>
  <div className="model-download-heading"><h3>{task.stem?t(`下载${stemLabel(task.stem,en)}模型`,`Downloading the ${stemLabel(task.stem,en)} model`):t('准备模型','Preparing models')}</h3><strong>{percent}%</strong></div>
  ...
  <ul className="model-download-queue">{rows.map(row=><li key={row.id}><span>{stemLabel(row.id,en)}</span><span className={row.ready?'ready':''}>{row.ready?t('已就绪','Ready'):row.id===task.stem?(task.phase==='failed'?t('下载中断','Interrupted'):t('下载中','Downloading')):t('等待下载','Waiting')}</span><small>{row.ready?formatModelBytes(row.total):`${formatModelBytes(row.received)} / ${formatModelBytes(row.total)}`}</small></li>)}{cachedTargets.length>0&&<li className="model-download-cached"><span>{cachedTargets.map(id=>stemLabel(id,en)).join(' / ')}</span><span className="ready">{t('已缓存','Cached')}</span><small>{t('无需重复下载','No download needed')}</small></li>}</ul>
 ...
 <svg aria-hidden="true" viewBox={`0 0 ${Math.max(1,peaks.length)} 100`} preserveAspectRatio="none"><path d={peaks.map((v,i)=>`M${i},${50-v*44}v${v*88}`).join(' ')} stroke="currentColor" strokeWidth="1"/></svg>
 </aside></div>
}
