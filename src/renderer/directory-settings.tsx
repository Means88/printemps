...
export function DirectorySettings({en,settings,onChange}:{en:boolean;settings:Settings;onChange:(settings:Settings)=>void}){
 ...
 return <section className="directory-settings">{(['modelDirectory','exportDirectory'] as const).map(kind=><div className="directory-setting" key={kind}><strong>{kind==='modelDirectory'?t('模型缓存目录','Model cache folder'):t('默认导出目录','Default export folder')}</strong><button className="directory-path" title={folders[kind]} disabled={busy} onClick={()=>choose(kind)}>{folders[kind]||t('导出时选择','Choose when exporting')}<span aria-hidden="true">…</span></button><div className="dialog-actions"><button disabled={busy||!settings[kind]} onClick={()=>choose(kind,true)}>{t('恢复默认','Use default')}</button></div></div>)}<small>{t('更改目录不会迁移已有模型。','Existing models are not moved.')}</small>{error&&<p role="alert">{error}</p>}</section>
}
