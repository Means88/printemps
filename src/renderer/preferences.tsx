...
export function Preferences({open,onOpenChange,en,settings,onChange}:{open:boolean;onOpenChange:(open:boolean)=>void;en:boolean;settings:Settings;onChange:(settings:Settings)=>void}){
 ...
 <label className="preference-row"><span><strong>{t('处理设备','Processing device')}</strong><small>{t('优先使用可用加速设备；不可用时使用 CPU。','Uses an available accelerator first and falls back to the CPU.')}</small></span><select disabled={saving} value={settings.device} onChange={e=>void save({device:e.target.value as Settings['device']})}><option value="auto">{t('自动选择','Automatic')}</option><option value="cpu">CPU</option><option value="cuda">NVIDIA CUDA</option><option value="mps">{t('Apple MPS（实验性）','Apple MPS (experimental)')}</option></select></label>
