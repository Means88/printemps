...
import {ImportRecovery} from './import-recovery'
...
import { PlayIcon, PauseIcon, ChevronRightIcon, ArrowLeftIcon, Pencil1Icon, TrackPreviousIcon, MixerHorizontalIcon, ViewVerticalIcon ,SpeakerLoudIcon,ArrowRightIcon} from '@radix-ui/react-icons'
...
import { TracksExportDialog } from './export-dialog'
...
import {beatGrid,formatPosition,rulerTicks,visibleTrackCount} from '../shared/timeline'
...
function App(){
 ...
 const [projects,setProjects]=useState<Project[]>([]),[project,setProject]=useState<Project|null>(null),[settings,setSettings]=useState<Settings>({language:'zh',device:'auto',modelDirectory:'',exportDirectory:'',proxyMode:'system',proxyUrl:''}),[settingsOpen,setSettingsOpen]=useState(false),[error,setError]=useState(''),[busy,setBusy]=useState(false),[search,setSearch]=useState(''),[playing,setPlaying]=useState(false),[time,setTime]=useState(0)
 ...
 async function editTrack(action:TrackAction){if(!project)return;if(session.saveError)throw session.saveError;session.mutate(project.id,()=>window.printemps.editTrack(project.id,action));await session.flush()}
 ...
 async function openProject(p:Project){try{await session.flush();session.open(await window.printemps.openProject(p.id));setImportError('');setHistoryOpen(false);setSelectedTrack('');setTime(0);setPlaying(false)}catch(e){if(e!==session.saveError)setError(String(e))}}
 async function importAudio(file?:File){if(busy)return;setBusy(true);setImportError('');try{await session.flush();const p=file?await window.printemps.importDroppedAudio(file):await window.printemps.importAudio();if(p){session.open(p);setTime(0);setPlaying(false);setProjects(await window.printemps.listProjects())}}catch(e){if(e!==session.saveError)setImportError(String(e))}finally{setBusy(false)}}
