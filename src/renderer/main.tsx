...
import { PlayIcon, PauseIcon, ArrowLeftIcon, Pencil1Icon, TrackPreviousIcon, MixerHorizontalIcon, ViewVerticalIcon } from '@radix-ui/react-icons'
...
function App(){
 ...
 function toggleLoop(){const duration=original?.duration||0;if(duration<.01)return;applyLoop(loop?null:marks??{start:0,end:duration})}
 function markLoop(side:'start'|'end'){
  const duration=original?.duration||0;if(duration<.01)return
 ...
 useEffect(()=>{
  let previous=''
  const sync=()=>{const source=project?.tracks.find(t=>t.id===selectedTrack);const state={language:settings.language,project:!!project,blocked:busy||loadingAudio||!!document.querySelector('[role="dialog"][data-state="open"]'),playing,loop:!!loop,metronome:!!project?.metronome,hasBpm:!!project?.music.bpm,canSeparate:!separationBusy&&!!source&&!source.hidden&&((project?.monitor==='original')===(source.role==='original')),inspector:!inspectorCollapsed};const key=JSON.stringify(state);if(key!==previous){previous=key;void window.printemps.syncMenu(state).catch(()=>{})}}
 ...
 },[project,settings.language,busy,loadingAudio,playing,loop,selectedTrack,inspectorCollapsed,separationBusy])
 ...
 },[project,loop,marks,busy,loadingAudio])
 useEffect(()=>{
