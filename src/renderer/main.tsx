...
function App(){
 ...
 async function editClip(action:ClipAction){if(!project||separationBusy||busy)return;const source=project.tracks.find(t=>t.id===action.trackId);if(!source||(project.monitor==='original')!==(source.role==='original'))return;setBusy(true);try{await session.flush();await window.printemps.editClip(project.id,action);session.refresh(project.id);await session.flush()}catch(e){setError(String(e))}finally{setBusy(false)}}
 ...
 const canSplit=clipActive&&!separationBusy&&!busy&&time>selectedClipData!.offset&&time<selectedClipData!.offset+selectedClipData!.end-selectedClipData!.start
