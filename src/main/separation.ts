...
export class SeparationService {
 ...
 async start(projectId:string,sourceId:string,ids:string[],cache:ModelCache,device:Settings['device']){
 ...
 private async execute(cache:ModelCache,ids:string[],source:Track,device:Settings['device'],controller:AbortController){
  ...
  try{
   ...
   if(disk.bavail*disk.bsize<source.duration*44100*8*(ids.length+1)*2+16*1024*1024)throw new Error('Insufficient disk space for separation results')
   const targets=[]
   ...
   for(const [i,file] of files.entries()){
    ...
    if(meta.sampleRate!==source.sampleRate||meta.channels!==source.channels||Math.abs(meta.duration-source.duration)>1/source.sampleRate)throw new Error('Separation result is not aligned with source')
 ...
 private async executeOriginal(projectId:string,source:Track,targets:{id:string;config:string;weight:string}[],directory:string,device:Settings['device'],signal:AbortSignal){
  ...
  for(const [index,target] of targets.entries()){
   ...
   const result=await this.runner(this.python,this.script,{input:this.store.assetPath(projectId,remainder?.assetId||source.assetId),output:chunkDirectory,device,targets:[target]},signal,p=>this.emit({phase:'separating',stem:target.id,progress:(index+p.progress)/targets.length}))
