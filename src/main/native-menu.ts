...
export function menuTemplate(s:MenuState,send:(command:MenuCommand)=>void,platform=process.platform):Item[]{
 ...
 {label:t('文件','File'),submenu:[command('import','导入音频…','Import audio…',!s.blocked,'CmdOrCtrl+O'),command('history','全部项目','All projects',!s.blocked,'CmdOrCtrl+Shift+O'),sep,command('export','导出剪辑…','Export clip…',active&&s.canExport,'CmdOrCtrl+Shift+E'),sep,{role:'close',label:t('关闭窗口','Close window')},...(!mac?[{role:'quit',label:t('退出','Quit')} as Item]:[])]},
 ...
 {label:t('音轨','Tracks'),submenu:[command('tracks','所有音轨…','All tracks…',active,'CmdOrCtrl+Shift+T'),command('separate','分离所选剪辑…','Separate selected clip…',active&&s.canSeparate),command('split','在游标处分割剪辑','Split clip at playhead',active&&s.canSplit),command('music','节奏与调性…','Rhythm and key…',active)]},
 {label:t('视图','View'),submenu:[command('zoomIn','放大时间轴','Zoom timeline in',active),command('zoomOut','缩小时间轴','Zoom timeline out',active),command('fit','显示完整音频','Fit audio',active),sep,command('inspector','剪辑详情','Clip details',active,undefined,s.inspector),sep,{role:'togglefullscreen',label:t('全屏','Full screen')}]},
