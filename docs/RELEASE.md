...
- 隐私政策：`site/privacy.html`（画板 34），部署后地址为 `https://printemps.dev/privacy.html`。文本按 Apple App 隐私详情结构撰写，向 App Store / Microsoft Store / 其它需要隐私条款链接的渠道提交时直接填这个地址；应用内数据处理方式变化时同步改页面并更新生效日期。
