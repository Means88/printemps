...
- Repository made public and v0.1.1 published (2026-09-21, at the user's instruction). Before flipping visibility the whole history was scanned: no key or credential file was ever added, none of `sntryu_`, `BEGIN PRIVATE KEY`, `ghp_`, `github_pat_`, `AKIA` or `xoxb-` appears in any commit, and no audio, checkpoint or installer was ever committed; the largest tracked files are design assets and `.git` is 68 MB. The draft was then published from `6d630a4`. Verified anonymously afterwards: the unauthenticated `releases/latest` API returns all ten assets, a ranged `curl` of the DMG answers 206, and printemps.dev shows `v0.1.1` with each platform card pointing at the right file. The committer email is public in the history from now on, which is normal for git but cannot be undone without rewriting it.
- Historical Linux packaging measurements, moved out of the README when it was trimmed to project info and basic workflow (2026-09-22). These predate the CPU-only dependency change and the 495 MB AppImage recorded above, so they are kept only as the record of what was measured at the time.
...
- CUDA guide rewritten for non-technical readers (user report + follow-up, 2026-09-21): the commands were written in cmd syntax while Windows 11 opens PowerShell by default, so pasting them produced `表达式或语句中包含意外的标记"install"` — `%USERPROFILE%` does not expand there and a quoted executable needs the call operator. The section now carries a shell switch: PowerShell (default) or Command Prompt, remembered in `printemps-shell` separately from the language preference, and every command exists in both syntaxes. On the user's point that the audience cannot be assumed to know how to open a terminal, the step is now shell-specific and concrete — which key to press, what to type, what to click, and what the prompt looks like when it worked — The step leads with Win+R and the Run box (the one route that does not shift with the Windows version or display language), keeping Start-menu search as the parenthetical alternative, plus paste-and-Enter guidance, a warning that the 2.4 GB step takes ten minutes and the window must stay open, and a copy button on every command block so nobody retypes a long URL. `tests/documentation-topics.test.ts` pins the syntax per shell (no `%VAR%` or call operator in the wrong one), that both shells carry their own how-to in both languages, and that the switch, its persistence and the copy button survive. Verified by toggling in the browser: the instructions and all three commands switch together, in both languages.
- Diagnostic log export (user decision after declining Sentry, 2026-09-21): rather than automatic crash reporting, Settings gained a 诊断日志 row that writes this session's failures to a file the user picks and attaches to a GitHub issue themselves. The app sends nothing, so the privacy policy's "not collected" position is unchanged; the policy and the guide now describe the feature instead. Failures are recorded centrally at the IPC boundary plus the separation and analysis task callbacks, into a bounded in-memory buffer (200 entries) that never touches disk unless exported. Because the file is destined for a public issue, `redact` strips the home directory, the bare account name, audio file names (keeping the extension) and project UUIDs; the report carries app/Electron/OS versions and the device, interpreter, download-source, proxy and language settings. Redaction is done by known segments rather than path grammar — an earlier attempt matched "everything after ~" and stopped at the first space, leaving `Application Support/.../song.wav` intact, which the tests now pin. A second bug caught by the tests: `path.basename` only splits on the host separator, so a Windows home read on macOS kept its backslashes and the account name survived; it now splits on both. `tests/diagnostics.test.ts` covers redaction on both platforms, the buffer bound, report contents and the empty-session case. Board 09 gained the row (1376 → 1507); guide §12 and §14 and the privacy policy's on-device list updated in both languages.
- Deep link from Settings to the guide (user request, 2026-09-21): the 推理环境 row gained a 查看配置说明 link that opens the CUDA walkthrough in the default browser. The renderer passes a fixed topic key, never a URL — main keeps a `DOCUMENTATION` allowlist, resolves the section and the current interface language itself, and calls `shell.openExternal`, so the renderer cannot ask the shell to open an arbitrary address. The guide now reads the language out of the deep link (`#en-12-cuda`) before rendering and scrolls afterwards, because the target sits inside the other language's article and would otherwise be `display:none` with nothing to scroll to. Verified both directions locally: `#en-12-cuda` switches the page to English and puts the heading at the top, `#zh-12-cuda` does the same in Chinese, and the link renders beside the interpreter status. `tests/documentation-topics.test.ts` ties the three sides together — every topic in the allowlist must have an `id` in both languages, the renderer must request a topic main knows, and the page must keep the language-from-hash logic.
- GPU backend naming (user request after reviewing AMD's Windows ROCm release notes, 2026-09-21): a ROCm PyTorch build answers `torch.cuda.is_available()` and is driven through `torch.cuda`, so an AMD user configuring their own environment would have seen an option labelled "NVIDIA CUDA". `device_probe.py` now also reports `backend`, derived from `torch.version.hip` / `torch.version.cuda`, and the renderer names the option, the "本机可用" line, the automatic option and the interpreter status from it through one shared `backendName` helper rather than four hardcoded strings. Unknown backend falls back to "NVIDIA CUDA / AMD ROCm" rather than assuming NVIDIA. Verified in the preview by temporarily reporting a ROCm probe: all four places switched to AMD ROCm in both languages, then the fixture was restored to the real CPU-only state. `tests/device-backend.test.ts` runs the probe against the bundled runtime and pins that `backend` is one of '', 'cuda', 'rocm' and that a true `cuda` always carries a backend name. Boards 09 and 35 and the guide's device wording updated. Not verified on AMD hardware; AMD's own notes make that path narrow anyway — Windows 11 only, a dedicated preview driver replacing the normal one, a short list of supported Radeon/Ryzen AI parts, and a known intermittent out-of-memory issue on Ryzen AI Max 300 that resembles the MPS thrashing `select_chunk` already works around.
- CUDA setup walkthrough on the guide (user request, 2026-09-21): §12 gained an `<h3>` subsection, zh and en, walking a Windows user through installing Python, creating a venv, installing CUDA PyTorch and pointing 推理环境 at it. Added as a subsection so §13 and §14 keep their numbers and anchors. Facts were checked against the PyTorch index rather than assumed: torch 2.11.0 has Windows CUDA wheels for cp310–cp314 across cu126/cu128/cu130, so the guide recommends 3.13 while noting the wider range, and tells the user to fall back to cu128/cu126 if the driver is too old. The section states plainly that the **CUDA Toolkit is not needed** (the wheels carry their own runtime) and that torchaudio is not needed either, since separation never imports it — both save the reader a multi-GB download. An earlier check wrongly concluded cp314 was missing; that came from a truncated listing and was corrected before writing. Also fixed a real validation gap found while listing the required packages: the vendored BS-Roformer imports `packaging`, which `device_probe.py` did not check, so an environment missing it would pass validation and fail later during separation. Verified rendering at 1024 and 375 px: three code blocks scroll inside their own box, no page overflow. Board 35 §12 and the board table updated.
