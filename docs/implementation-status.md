...
| Other 必须显示；二次分离原子替换 [Other, targets] | domain/service 测试，真实逐步及二次分离重建误差 <1e-5、原位替换及混音设置继承 | macOS 二次分离原位替换已通过；取消时的完整桌面行为待验 |
...
| Windows/macOS/Linux | native build 工作流；macOS arm64 测试包；Linux x64 容器内 AppImage 及包内分析通过 | 三平台 CI 构建和包内分析已通过；最新缓存配置 CI 仍运行，各平台 GUI、安装/卸载与升级待验 |
