...
用户已手动退出屏保，重建自动化连接后已确认可读取 Printemps 原生界面，正在执行真实 Electron 验收。已完成单音频导入、项目/音轨改名、手动触发分析、正常退出重开、按需模型下载、首轮及二次分离、WAV/FLAC 导出与播放时钟推进。详细证据见 `acceptance.md`。Windows/macOS/Linux 的 c7d60f0 已通过三平台未安装目录构建和包内分析。e29bc55 的安装包工作流 35482306981 已三平台全部成功，包含 Windows NSIS、macOS DMG/ZIP、Linux AppImage 及各平台包内分析。之后的导出目录入口与恢复边界修复尚未运行三平台 CI。尚未验证真实可听输出、Windows/Linux GUI、安装升级及发布服务器；未发布版本。正式发布还需依赖许可/声明与签名、公证。
...
1. 安装包工作流 35482306981 已三平台全部成功；后续改动仍需最终构建验证。
2. 已原生验收的 macOS 包仍为 f0c86c5；4648ce4 已构建到 release-status，签名校验及包内分析通过，已原生启动并重开项目。用户已确认导出正常，成功后的“打开文件夹”已原生确认打开导出副本目录；任务恢复和实际音频输出仍待验。
