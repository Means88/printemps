...
Mac 已解锁，正在执行真实 Electron 验收。已完成单音频导入、项目/音轨改名、手动触发分析、正常退出重开、按需模型下载、首轮及二次分离、WAV/FLAC 导出与播放时钟推进。详细证据见 `acceptance.md`。Windows/macOS/Linux CI 已完成构建和包内分析，最近已验证的提交为 6b453e5，之后的改动尚未运行三平台 CI。尚未验证真实可听输出、Windows/Linux GUI、安装升级及发布服务器；未发布版本。正式发布还需依赖许可/声明与签名、公证。
...
2. 当前 macOS 测试包为 f0c86c5，S/M 状态和循环操作已原生确认；继续验证实际音频输出。
