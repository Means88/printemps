...
test('directory preferences preserve concurrent language edits and reject private or busy cache targets',async()=>{
 ...
 try{
  ...
  await settings.save({hfEndpoint:' hf-mirror.com '})
  expect((await store.settings()).hfEndpoint).toBe('https://hf-mirror.com')
  expect(()=>settings.save({hfEndpoint:'https://mirror/path'})).toThrow('host and port only')
  await settings.save({hfEndpoint:''})
  expect((await store.settings()).hfEndpoint).toBe('')
