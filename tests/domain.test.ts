...
describe('Project invariants',()=>{
 it('replaces A at its index with remainder C then B, retaining neighbours',()=>{
  const p=make();p.tracks[2].muted=true
  const next=commitSeparation(p,p.tracks[2].id,[track('stem','B'),track('other','C')])
  expect(next.tracks.map(t=>t.name)).toEqual(['Original','X','C','B','Y'])
...
it('merges delayed metadata edits without removing new results or resurrecting replaced tracks',()=>{
 ...
 expect(merged.tracks.map(t=>t.name)).toEqual(['Original','Renamed X','C','B','Y'])
